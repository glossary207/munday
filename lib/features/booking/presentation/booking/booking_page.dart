...
class _BookingWidgetState extends ConsumerState<BookingPage> {
  ...
  List<String> _selectedTableIds = const [];
  ...
  void _handleDateSelection() {
    ...
    if (selected == null ||
        (selected.year == _selectedDate.year &&
            ...
            selected.day == _selectedDate.day)) {
      return;
    }
    setState(() {
  ...
  void _handleLayoutSelection(
    ...
    if (!mounted) return;
    setState(() {
      _selectedTableIds = List.unmodifiable(tableIds);
      _minimumPartySize = minimumPartySize < 1 ? 1 : minimumPartySize;
    });
    context.appState.update(() {
      context.appState.SeatSelect = List.of(displayNames);
      context.appState.Totlepricebooking = totalPrice.round();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.id == null) {
      return const Scaffold(
        ...
        body: Center(
          child: Text(
            'ไม่พบข้อมูลร้าน กรุณากลับไปเลือกร้านอีกครั้ง',
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'โหลดข้อมูลร้านสำหรับจองโต๊ะไม่สำเร็จ',
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          child: custom_widgets.LayoutPreviewWidget(
                            ...
                            venueId: widget.id!.id,
                            date: _selectedDate,
                            floorId: widget.floorId.trim().isEmpty
                                ? 'F1'
                                : widget.floorId,
                            onSelectionChanged: _handleLayoutSelection,
                          ),
                        ),
                      ),
                      Container(
                                ...
                                onTap: () async {
                                  if (_selectedTableIds.isEmpty) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text(
                                          'กรุณาเลือกโต๊ะก่อนดำเนินการต่อ',
                                        ),
                                      ),
                                    );
                                    return;
                                  ...
                                  context.pushNamed(
                                    ...
                                    queryParameters: {
                                      ...
                                      'tableIds': serializeParam(
                                        _selectedTableIds,
                                        ParamType.String,
                                        isList: true,
                                      ),
                                      'amount': serializeParam(
                                        context.appState.Totlepricebooking
                                            .toDouble(),
                                        ParamType.double,
                                      ),
                                      'partySize': serializeParam(
                                        _minimumPartySize,
                                        ParamType.int,
                                      ),
                                    }.withoutNulls,
