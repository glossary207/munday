...
String _serializeSupabaseDocRef(SupabaseDocRef ref) {
  final docIds = <String>[];
  SupabaseDocRef? currentRef = ref;
  while (currentRef != null) {
    docIds.add(currentRef.id);
    // Get the parent document (catching any errors that arise).
    currentRef = safeGet<SupabaseDocRef?>(() => currentRef?.parent.parent);
  }
...
String? serializeParam(
  ...
  try {
    ...
    switch (paramType) {
      ...
      case ParamType.SupabaseDocRef:
        data = _serializeSupabaseDocRef(param as SupabaseDocRef);
      case ParamType.Document:
        final reference = (param as SupabaseRecord).reference;
        data = _serializeSupabaseDocRef(reference);

      case ParamType.DataStruct:
        data = param is BaseStruct ? param.serialize() : null;

      case ParamType.Enum:
        data = (param is Enum) ? param.name : null;
...
SupabaseDocRef _deserializeSupabaseDocRef(
  String refStr,
...
dynamic deserializeParam<T>(
  ...
  try {
    ...
    switch (paramType) {
      ...
      case ParamType.SupabaseDocRef:
        return _deserializeSupabaseDocRef(param, collectionNamePath ?? []);
