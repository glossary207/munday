...
class _MapEventState extends State<MapEvent>
  ...
  void _initializeMap() async {
    ...
    _setMarkers();
    _setCircles();
    if (FFAppState().EventSelection != null) {
      _moveToSelectedEvent();
  ...
  void _setMarkers() {
    ...
    if (widget.items != null) {
      for (final event in widget.items!) {
        if (event.location != null) {
          ...
          final isSelected = FFAppState().EventSelection != null &&
              FFAppState().EventSelection == event.reference;
  ...
  Future<void> _moveToSelectedEvent() async {
    if (FFAppState().EventSelection != null) {
      final eventRecord =
          await EventsRecord.getDocumentOnce(FFAppState().EventSelection!);
      if (eventRecord != null && eventRecord.location != null) {
        final controller = await _controller.future;
        final position = gmf.LatLng(
          eventRecord.location!.latitude,
          eventRecord.location!.longitude,
        );
  ...
  Future<void> _moveToCurrentLocation() async {
    if (widget.currentLocation != null) {
      ...
      // Run animation when moving to current location
      _runAnimation();
    }
  }
  ...
  @override
  void didUpdateWidget(covariant MapEvent oldWidget) {
